...
# HabitCoach AI — Product Requirements Document
...
- Widok historii pokazuje 30 dni dla każdego nawyku w sposób umożliwiający zobaczenie wzorca (siatka dni z oznaczeniem wykonane / niewykonane).
- Generowanie rekomendacji jest jednym kliknięciem; wynik pojawia się w sposób kontekstowy i odnosi się do nazwy nawyku użytkownika lub konkretnego wzorca wykonania.
- Pusty dashboard (brak nawyków) i pusta historia (brak logowań) mają jawne empty-state z instrukcją następnego kroku.
...
- FR-007: Zalogowany użytkownik może zarchiwizować nawyk (nawyk znika z list aktywnych, historia wykonań zachowana dla AI). Priority: must-have
  > Socrates: Kontrargument rozważony: "twarde usuwanie z historią traci dane dla AI". Rezolucja: FR zmienione z 'usuń' na 'zarchiwizuj' — historia zostaje, AI ma większą bazę. Hard delete (np. żądanie GDPR) odłożone.
...
- FR-010: Zalogowany użytkownik może przeglądać historię wykonań każdego nawyku za ostatnie 30 dni. Priority: must-have
  > Socrates: Kontrargument rozważony: "30 dni za dużo, tygodniowy widok wystarczy". Rezolucja: zostaje 30 dni — AI potrzebuje dłuższej historii, by rekomendacje były specyficzne.
...
Użytkownik napotyka rekomendację w dwóch momentach: (a) gdy świadomie ją wygeneruje (FR-011), (b) gdy aplikacja sama ją pokaże po osiągnięciu progu zbioru danych (FR-013). Po zobaczeniu rekomendacji użytkownik wraca do dashboardu i kontynuuje logowanie nawyków — nie ma w MVP innych przepływów oddzielnie zarządzających rekomendacjami.

## Access Control

Uwierzytelnianie: email + hasło. Rejestracja konta, logowanie, możliwość odzyskania hasła (reset przez link na email). Sesja użytkownika utrzymywana po zalogowaniu (długa sesja domyślnie, "remember me").
...
- Integracje z zewnętrznymi platformami fitness / health (trackerami aktywności, smartwatchami, aplikacjami zdrowotnymi) — brak automatycznego logowania snu / treningu z urządzeń zewnętrznych.
- Gamifikacja (punkty, odznaki, rankingi, poziomy) — brak warstwy nagród poza widokiem własnej historii.
- Aplikacje mobilne natywne — MVP jest tylko web; natywne odłożone.
- Współdzielenie nawyków między użytkownikami (link-share, team workspaces, publiczny profil) — produkt pozostaje single-user.
- Wsparcie częstotliwości nawyków innych niż codzienna ("N razy w tygodniu", "tylko w dni robocze") — wszystkie nawyki w MVP są codzienne; rytmy złożone odłożone do v2.
- Historia wygenerowanych rekomendacji w UI — użytkownik widzi tylko ostatnią; archiwum rekomendacji odłożone.
- Logowanie wykonań wstecz oraz cofanie wstecz (poza dniem bieżącym) — zakres ograniczony, by streaki pozostawały realne.
- Hard delete nawyku wraz z historią — w MVP jest tylko archiwizacja zachowująca historię dla AI.

## Open Questions

1. **Próg danych dla automatycznej rekomendacji (FR-013)** — wstępnie ≈ 7 dni logowań dla co najmniej jednego nawyku, ale precyzyjna definicja (czy "co najmniej jednego" wystarczy? jaki minimum count wykonań?) wymaga doprecyzowania przed implementacją. Owner: user. Block: nie (można zacząć z prowizorycznym progiem, doprecyzować przed deploymentem).
2. **Sposób pomiaru kryterium "75 % rekomendacji odnosi się do konkretnych danych użytkownika"** — czy weryfikacja jest manualna (przegląd próbki rekomendacji), automatyczna (np. sprawdzenie, że rekomendacja zawiera nazwę nawyku użytkownika lub konkretny token z danych), czy obie? Wymaga decyzji przed startem MVP, by mierzyć Primary success criterion. Owner: user. Block: tak (bez metody pomiaru, kryterium sukcesu Primary nie jest weryfikowalne).
   > **RESOLVED (2026-06-20, przed planowaniem S-04):** Metoda **hybrydowa** — automatyczny token-check przy każdej generacji (rekomendacja zawiera ≥1 konkretny token z danych usera: nazwa nawyku, liczba w streaku, dzień słabszy, data przerwy) jako metryka operacyjna liczona z logów, kalibrowana okresowym ręcznym przeglądem próbki (czy token = realne ugruntowanie). Check jest **obserwacyjny, nie bramkujący** — rekomendacja zawsze trafia do usera, wynik token-checka jest logowany jako metryka (iteracja na promptcie), bez regeneracji (chroni NFR <10s + koszt). Token-set i logowanie metryki implementowane w S-04.
3. **Częstotliwość rekomendacji automatycznej (FR-013)** — czy aplikacja generuje proaktywną rekomendację tylko raz po osiągnięciu progu, czy regularnie (np. co N dni / co tydzień)? Owner: user. Block: nie (MVP może zacząć od jednorazowej rekomendacji proaktywnej; cykliczność doprecyzowana w v1.x).
