...
Bramka jest **pobłażliwa, tylko ostrzegawcza** — `/10x-archive` blokuje tylko w przypadku niezacommitowanych zmian w folderze zmiany. Wszystko inne (niekompletny Progress, brak impl-review, status nie w `{implemented, impl_reviewed}`) jest wyświetlane jako ostrzeżenie, po którym następuje monit potwierdzenia; użytkownik nadal może archiwizować.
...
1. Usuń początkowe `@`, jeśli jest obecne.
2. Usuń końcowe `/`, jeśli jest obecne.
...
**2. Istniejące już zmiany w stagingu gdziekolwiek.** Krok commitowania archiwum (patrz "Przenieś i oznacz" poniżej) łączy wszystko, co jest w stagingu w momencie commitowania. Jeśli użytkownik ma niezwiązane zmiany w stagingu z wcześniejszej pracy, trafiłyby one cicho do commita `chore(archive): close ...`. Uruchom:
...
5. **Zamknij pasujący element planu** — najlepszy wysiłek; ten krok nigdy nie blokuje, nigdy nie cofa i nigdy nie monituje. Plan jest opcjonalny; większość zmian nie będzie się do niego odnosić.
   ...
   3. Odczytaj `context/foundation/roadmap.md`. Poszukaj `<change-id>` użytego jako `Change ID`:
      ...
      `<ID>` to lokalny identyfikator tego elementu w planie (`F-NN` lub `S-NN`); `<Outcome>` to tekst jego linii `- **Outcome:**` (zachowaj początkowe `(foundation) `, jeśli jest obecne).
   4. **Brak dopasowania** → wydrukuj `ℹ context/foundation/roadmap.md nie ma elementu z Change ID "<change-id>" — plan pozostawiony bez zmian.` i pomiń resztę tego kroku. Dopasowanie jest tylko dokładnym ciągiem; wycinek planu może generować kilka zmian, więc bliskie dopasowanie celowo *nie* jest zamykane.
...
6. **Zacommituj archiwum.** Utwórz jeden commit:
   ...
   Bez treści — temat jest mechaniczny, a diff (zmiana nazwy + znacznik frontmatter, plus zamknięcie planu, gdy pasowało) jest oczywisty. Nigdy nie przekazuj flag `--no-verify` ani flag omijających podpisywanie. Jeśli hook pre-commit zawiedzie, napraw podstawowy problem i utwórz NOWY commit.
...
- Nie dodaje SHA do elementów Progress — `/10x-implement` jest jedynym autorem sufiksu SHA na końcu fazy. Bramka archiwum wymusza obecność SHA jako sygnał tylko ostrzegawczy (patrz miękkie sprawdzenie ostrzegawcze 4); nigdy nie przepisuje wiersza bez SHA.
