...
3. **Sprawdzenie braku impl-review**: glob `context/changes/<change-id>/reviews/impl-review*.md`. Jeśli żaden nie pasuje, dodaj do kolejki: `No impl-review found at reviews/impl-review*.md.`
4. **Sprawdzenie braku SHA**: przeanalizuj sekcję `## Progress` pliku `plan.md` (jeśli istnieje). Policz wiersze `- [x]`, których linia NIE kończy się na ` — <sha>`, gdzie `<sha>` to 7+ znaków szesnastkowych (tj. wyrażenie regularne ` — [0-9a-f]{7,}$` nie pasuje). Jeśli liczba jest różna od zera, dodaj do kolejki: `<N> Progress rows missing SHA suffix: <lista tokenów "N.M <title>" oddzielonych przecinkami, obcięta do 5 z "…" jeśli dłuższa>.` Wiersze bez SHA są uzasadnione dla faz z pustym diffem i dla planów, które zostały ukończone przed wprowadzeniem kontraktu SHA — jest to miękki sygnał, a nie wada. Pomiń cicho, jeśli brakuje `plan.md` (sprawdzenie oczekującego postępu już objęło ten przypadek).
...
5. **Zamknij pasujący element roadmapy** — najlepszy wysiłek; ten krok nigdy nie blokuje, nigdy nie cofa i nigdy nie monituje. Roadmapa jest opcjonalna; większość zmian nie będzie do niej prowadzić.
   ...
   3. Odczytaj `context/foundation/roadmap.md`. Poszukaj `<change-id>` użytego jako `Change ID`:
      ...
      `<ID>` to lokalny identyfikator tego elementu roadmapy (`F-NN` lub `S-NN`); `<Outcome>` to tekst jego linii `- **Outcome:**` (zachowaj początkowe `(foundation) `, jeśli jest obecne).
   4. **Brak dopasowania** → wydrukuj `ℹ context/foundation/roadmap.md has no item with Change ID "<change-id>" — roadmap left untouched.` i pomiń resztę tego kroku. Dopasowanie jest tylko dokładnym ciągiem; wycinek roadmapy może generować kilka zmian, więc bliskie dopasowanie celowo *nie* jest zamykane.
...
6. **Zatwierdź archiwum.** Utwórz jeden commit:
   ...
   Bez treści — temat jest mechaniczny, a diff (zmiana nazwy + znacznik frontmatter, plus zamknięcie roadmapy, gdy pasuje) jest oczywisty. Nigdy nie przekazuj flag `--no-verify` ani flag pomijających podpisywanie. Jeśli hook pre-commit zawiedzie, napraw podstawowy problem i utwórz NOWY commit.
...
- Nie dodaje SHA do elementów Progress — `/10x-implement` jest jedynym autorem sufiksu SHA na końcu fazy. Brama archiwum wymusza obecność SHA jako sygnał tylko ostrzegawczy (patrz miękkie sprawdzenie ostrzegawcze 4); nigdy nie przepisuje wiersza bez SHA.
