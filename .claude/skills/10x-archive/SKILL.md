...
Zbierz następujące ostrzeżenia, a następnie przedstaw je wszystkie naraz z pojedynczym monitem potwierdzenia.
...
Jeśli nie było ostrzeżeń w kolejce, pomiń monit i przejdź bezpośrednio.
...
5. **Zamknij pasujący element roadmapy** — najlepszy wysiłek; ten krok nigdy nie blokuje, nigdy nie cofa i nigdy nie prosi o potwierdzenie. Roadmapa jest opcjonalna; większość zmian nie będzie do niej prowadzić.
   ...
   2. Sprawdź, czy plik jest już brudny: `ROADMAP_PREDIRTY=$(git status --porcelain context/foundation/roadmap.md 2>/dev/null)`. (Używane w podkroku 7 do podjęcia decyzji, czy umieścić go w commicie archiwum.)
   ...
   4. **Brak dopasowania** → wydrukuj `ℹ context/foundation/roadmap.md nie ma elementu z Change ID "<change-id>" — roadmapa pozostawiona bez zmian.` i pomiń resztę tego kroku. Dopasowanie jest tylko dokładnym ciągiem; fragment roadmapy może generować kilka zmian, więc bliskie dopasowanie jest celowo *nie* zamykane.
   5. **Znaleziono dopasowanie** → zastosuj trzy edycje poniżej za pomocą narzędzia Edit. Każda jest niezależna i stanowi najlepszy wysiłek: jeśli cel nie znajduje się tam, gdzie umieszcza go szablon `/10x-roadmap` (roadmapa edytowana ręcznie, starszy format), pomiń tę podedycję, kontynuuj i zanotuj, co zostało pominięte — nigdy nie przerywaj archiwizacji z powodu kształtu roadmapy. Dotykaj tylko pól wymienionych tutaj; pozostaw `Outcome`, `Prerequisites`, `Parallel with`, `Risk` itp. bez zmian.
...
roadmap.md:     zamknięto <ID> "<Outcome>"  →  Status: done, wpis dodany do ## Done    ← drukuj tylko, gdy element roadmapy pasował; w przeciwnym razie pomiń tę linię
...
- Krok zamykania roadmapy ("Przenieś i oznacz", krok 5) jest izolowany: każda awaria jest wychwytywana, odnotowywana w danych wyjściowych potwierdzenia i pomijana. Nigdy nie przerywa archiwizacji i nigdy nie wywołuje wycofania. Częściowo zastosowana edycja roadmapy jest możliwa do odzyskania ręcznie.
