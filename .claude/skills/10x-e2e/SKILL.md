...
- **Wykonuje jeden przejrzany test na ryzyko, a nie pełne skanowanie.** W przeciwieństwie do "generowania testów dla każdej strony", ta umiejętność pisze mały, związany z ryzykiem zestaw i wzmacnia każdy z nich poprzez przegląd i celowe sprawdzenie błędu. E2E to najdroższa, najbardziej podatna na błędy warstwa — liczba pokrycia nigdy nie jest celem; chronione ryzyko jest.
- **Każda faza jest sprawdzana pod kątem tego, czy E2E faktycznie pasuje i czy aplikacja jest gotowa.** Niektóre fazy (czysta logika, konfiguracja, szkieletowanie) nigdy nie powinny mieć testu E2E. Funkcje, które nie są zbudowane, nie mogą być uruchamiane w przeglądarce. Te przypadki są przekierowywane lub zatrzymywane, jak opisano poniżej.
...
> Samodzielne uruchomienia (oparte na ryzyku) pomijają całą tę sekcję — przechodzą bezpośrednio do bramki.
...
6. **Upewnij się, że istnieją dwie dźwignie jakości (jednorazowa konfiguracja dla każdego projektu).** To one wykonują ciężką pracę — prompt pozostaje cienki.
   - **Test początkowy** (`seed.spec.ts`): egzemplarz, na którym wzorowany jest każdy generowany test. *Co pokazujesz, to dostajesz* — jeśli seed używa `getByRole`, generowane testy również; jeśli ma `waitForTimeout`, każdy generowany test to dziedziczy. Jeśli brakuje, utwórz go z `references/seed-test-pattern.md`, dostosowanego do rzeczywistych tras i ról tej aplikacji. Zobacz także `references/browser-driven-generation.md`.
   ...
   - Traktuj oba jako część zestawu plików dotkniętych **pierwszej fazy**, aby znalazły się w zatwierdzeniu tej fazy. Gdy już istnieją, zostaw je — nie twórz ich ponownie w każdej fazie.

7. **Zaktualizuj `change.md`**: ustaw `status: implementing` (tylko jeśli aktualnie w `{planned, plan_reviewed}`) i `updated: <dzisiaj>`.

8. **Utwórz jedno zadanie na fazę** (pojawiają się one na pasku stanu użytkownika): dla każdego nagłówka `## Phase N:`, który zamierzasz uruchomić, `TaskCreate` z `subject: "Phase N: [Nazwa Fazy]"` i `activeForm: "E2E Faza N"`. Oznacz bieżącą fazę jako `in_progress` przed rozpoczęciem; oznacz ją jako `completed`, gdy spełnione zostaną jej kryteria sukcesu.
...
## Bramka kwalifikacyjna E2E — uruchamiana przed każdą fazą (oparta na planie) / raz (samodzielna)
...
| Ryzyka, które pojawiają się tylko wtedy, gdy integrują się rzeczywiste granice | Konfiguracja, szkieletowanie, okablowanie infrastruktury, dokumentacja |
...
Szybko wyszukaj istniejący spec obejmujący to ryzyko. Jeśli **przechodzący** test E2E dla ryzyka już istnieje, nie generuj go ponownie — zaznacz wiersz Postępu i przejdź dalej (lub, jeśli jest mieszany, zapytaj). Jeśli test istnieje, ale **zawodzi**, jest to zadanie debugowania, a nie generowania — skieruj użytkownika do przepływu pracy debugowania od nieudającego się testu do głównej przyczyny, zamiast pozwalać narzędziu do automatycznej naprawy cicho przepisywać asercję. (Zobacz granicę automatycznej naprawy w wytycznych E2E.)
...
Nigdy nie używaj `test.skip()` / `test.fixme()` do "przejścia" fazy — pominięty test jest niewidoczny. Test, którego nie można zmusić do przejścia na prawdziwej aplikacji, jest sygnałem do zbadania (funkcji, przepływu lub błędu), a nie do wyciszenia.
...
W uruchomieniu **samodzielnym** nie ma fazy ani `## Progress`: po tym, jak WERYFIKUJ potwierdzi zielony wynik plus celowe uszkodzenie, **zatrzymaj się** — zgłoś plik specyfikacji i ryzyko, które chroni, i pomiń rytuał zakończenia fazy poniżej.
...
Utrzymuj **zestaw dotkniętych plików** przez całą fazę: każdy plik, który `Edytujesz`/`Zapisujesz` (specyfikacje, plik promptu, a w pierwszej fazie seed + dźwignie reguł) trafia do niego, plus `context/changes/<change-id>/plan.md` (zawsze — edytujesz jego Postęp). W **pierwszej fazie** zmiany, również zasil go wszystkimi nieśledzonymi/zmodyfikowanymi plikami w `context/changes/<change-id>/` (`change.md`, `research.md` itp.). Zestaw **resetuje się na każdej granicy fazy**.

1. **Uruchom specyfikację E2E fazy** (lub specyfikacje) na działającej aplikacji i potwierdź zielony wynik. (Pełne przejście E2E odbywa się w CI, a nie na każdą edycję — lokalnie potwierdzasz specyfikację (specyfikacje) dodane w tej fazie. Napraw wszelkie błędy przed zatwierdzeniem.)
...
```
   ...
   W **ostatniej fazie**, również zsumuj wszystkie nadal oczekujące wiersze `#### Manual` z wcześniejszych faz (informacyjne; bramka nadal tylko wstrzymuje, nie blokuje twardo).
...
6. **Zaproponuj wiadomość Conventional-Commits** i zatwierdź ją za pomocą `AskUserQuestion` (zatwierdź jako zaproponowane / edytuj temat / nadpisz). Temat: `test(<change-id>): <tytuł fazy> (p<N>)`. W treści wspomnij o charakterze E2E/poziomu przeglądarki i chronionym ryzyku. Dołącz linię `Refs:`, jeśli rozmowa zawiera rzeczywiste odniesienia Jira/Linear/GitHub (nigdy nie wymyślaj ich z change-id lub gałęzi).
...
**Kontynuuj:** przeczytaj następną fazę, ustaw jej zadanie na `in_progress`, uruchom bramkę E2E, kontynuuj. Nie ma potrzeby ponownego czytania całego planu.

**Przejrzyj:** uruchom `/10x-impl-review @<ścieżka-do-planu> phase [N]`, a następnie ponownie przedstaw decyzję kontynuuj/wyczyść (bez opcji przeglądu).
...
- **Chroń nazwane ryzyko, a nie obszar powierzchni** — bez testu na stronę/przycisk, bez nadmiernego mockowania wewnętrznych granic (mockowanie uwierzytelniania + DB i test niczego nie sprawdza, co może się zepsuć w integracji), bez asercji pikseli dla ryzyk funkcjonalnych (użyj do tego deterministycznych narzędzi wizualnych).

**Rzeczywiste vs mockowane** to podstawowa wartość testu: wewnętrzne granice (uwierzytelnianie, routing, DB) pozostają rzeczywiste — tam ukrywa się ryzyko integracji; mockuj tylko drogie lub niedeterministyczne zewnętrzne API na warstwie sieciowej.

**Wizja** (`--caps=vision`) jest uzupełnieniem dla ryzyk wyłącznie wizualnych (układ, z-index, animacja, canvas), a nie domyślnym — migawki DOM weryfikują funkcję. **Narzędzia do automatycznej naprawy** pomagają w dryfie selektora/czasu (kieruj ich wynik przez przegląd PR, nigdy nie zatwierdzaj automatycznie), ale nigdy nie mogą "naprawić" *zmienionego zachowania biznesowego* — to maskuje regresję, którą test ma wychwycić. Oba są szczegółowo opisane w `references/browser-driven-generation.md`; nieudany test E2E to zadanie debugowania, a nie generowania ani naprawy.
...
- `references/browser-driven-generation.md` — samodzielne sterowanie przeglądarką w celu planowania i generowania jednej specyfikacji na ryzyko (przepływ pracy drzewa dostępności, migawka zamiast zrzutów ekranu, jeden test na plik, zapis z rzeczywistego wykonania, granica automatycznej naprawy).